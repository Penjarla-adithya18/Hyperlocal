...
import { User, Briefcase, Loader2, Phone, ShieldCheck, Lock, Building2, Store, CheckCircle2, AlertTriangle, Mail } from 'lucide-react';
...
function SignupPageContent() {
  ...
  const [bypassAadhaarVerification, setBypassAadhaarVerification] = useState(false);
  ...
  // ── Aadhaar KYC — auto-verify at 12 digits ──────────────────────────
  const handleAadhaarChange = (value: string) => {
    const digits = value.replace(/\D/g, '').slice(0, 12);
    setAadhaarNumber(digits);
    setAadhaarVerified(digits.length === 12);
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!aadhaarVerified && !bypassAadhaarVerification) {
      toast({
        title: 'Aadhaar Required',
        description: 'Please enter your 12-digit Aadhaar number or bypass to continue.',
        variant: 'destructive',
      });
      return;
    }
              ...
              <div className="space-y-6 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 sm:p-5 dark:border-slate-700 dark:bg-slate-900/50">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 sm:p-5 dark:border-slate-700 dark:bg-slate-900/50">
                    ...
                    <div className={`group relative rounded-xl border bg-white px-3 py-2 dark:bg-slate-950 ${
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          maxLength={12}
                          className="relative block w-full appearance-none border-0 bg-transparent px-3 py-2 pl-10 text-base text-gray-900 placeholder-gray-400 font-mono tracking-wider transition-colors focus:outline-none focus:ring-0 dark:text-slate-100 dark:placeholder:text-slate-500"
                        />
                    ...
                    {aadhaarVerified && (
                      <div className="rounded-xl border border-emerald-200 bg-emerald-50/80 p-3 text-sm text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300">
                        <div className="flex items-center gap-2">
                          <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-600" />
                          <p className="font-medium">Aadhaar verified automatically</p>
                        </div>
                      </div>
                    )}

                    {!aadhaarVerified && aadhaarNumber.length > 0 && aadhaarNumber.length < 12 && (
                      <p className="text-xs text-gray-500 dark:text-slate-400">
                        {12 - aadhaarNumber.length} more digit{12 - aadhaarNumber.length !== 1 ? 's' : ''} needed to verify
                      </p>
                    )}
                    ...
                    <div className="flex items-start gap-3 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-950">
                      <Checkbox
                        id="bypassAadhaar"
                        checked={bypassAadhaarVerification}
                        onCheckedChange={(checked) => setBypassAadhaarVerification(checked === true)}
                        className="mt-0.5"
                      />
                      <label htmlFor="bypassAadhaar" className="cursor-pointer text-sm leading-relaxed text-gray-600 dark:text-slate-400">
                        Bypass Aadhaar verification for now and continue signup.
                      </label>
                    </div>
                  </div>
                </div>
